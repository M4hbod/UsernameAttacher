...
class Database:
    ...
    def get_user_dict(self, chat_id: int):
        return dict(id=chat_id, join_date=datetime.date.today().isoformat())

    def get_channel_dict(self, chat_id: int, defult_type: str):
        return dict(
            id=chat_id,
            join_date=datetime.date.today().isoformat(),
            settings=dict(enabled=False, mode=defult_type, username="default"),
        )
    ...
    async def enable(self, chat_id: int):
        ...
        channel = dict(enabled=True, mode=mode, username=username)
        await self.col.channels.update_one(
            {"id": chat_id}, {"$set": {"settings": channel}}
        )
    ...
    async def disable(self, chat_id: int):
        ...
        mode = channel["mode"]
        username = channel["username"]
        channel = dict(enabled=False, mode=mode, username=username)
        await self.col.channels.update_one(
            {"id": chat_id}, {"$set": {"settings": channel}}
        )
    ...
    async def set_mode(self, chat_id: int, mode: str):
        ...
        username = channel["username"]
        channel = dict(enabled=status, mode=mode, username=username)
        await self.col.channels.update_one(
            {"id": chat_id}, {"$set": {"settings": channel}}
        )
